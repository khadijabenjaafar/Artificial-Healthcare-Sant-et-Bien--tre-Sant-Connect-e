...
final class MatchingController extends AbstractController
{
    ...
    public function getFreelancerDetails($id, EntityManagerInterface $entityManager, LoggerInterface $logger): JsonResponse
    {
        try {
            ...
            if (strtolower($freelancer->getRole()) !== 'freelancer') {
                throw new \Exception("User with ID $id is not a freelancer.");
            }
    
            $matching = $matchingRepo->findOneBy(['utilisateur' => $freelancer]);
            if (!$matching) {
                $logger->info("No matching record found for freelancer ID: $id");
                $matching = new Matching(); // Prevent null reference errors
            } else {
                // Debugging: Log the matching details
                $logger->info("Matching details: " . json_encode([
                    'description' => $matching->getDescription(),
                    'competences' => $matching->getCompetences(),
                    'cv' => $matching->getCv()
                ]));
            }
