...
public class AfficheFacturation implements Initializable {
    ...
    private void showModificationForm(Facturation facturation) {
        ...
        Stage modalStage = new Stage();
        modalStage.initModality(Modality.APPLICATION_MODAL);
        modalStage.setTitle("Modifier Facturation #" + facturation.getId());
        ...
        // Titre avec icône
        HBox titleBox = new HBox(10);
        Label title = new Label("Modification de la Facturation");
        ...
        formCard.setStyle("-fx-background-color: white; -fx-background-radius: 10; -fx-border-radius: 10; " +
                "-fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.1), 10, 0, 0, 0);");

        // Champ Montant
        VBox montantBox = new VBox(5);
        Label montantLabel = new Label("Montant (TND)");
        montantLabel.setStyle("-fx-font-size: 14px; -fx-text-fill: #34495e;");
        TextField montantField = new TextField(String.valueOf(facturation.getMontant()));
        montantField.setStyle("-fx-font-size: 14px; -fx-padding: 8px; -fx-background-radius: 5;");
        montantBox.getChildren().addAll(montantLabel, montantField);

        // Affichage des autres informations (non modifiables)
        VBox infoBox = new VBox(10);
        infoBox.setPadding(new Insets(10, 0, 0, 0));

        Label infoTitle = new Label("Informations complémentaires (non modifiables)");
        infoTitle.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-text-fill: #7f8c8d;");

        HBox dateBox = createInfoRow("📅 Date:", facturation.getDateFacturation().toString());
        HBox methodeBox = createInfoRow("💳 Méthode:", facturation.getMethodePaiement());
        HBox statutBox = createInfoRow("📌 Statut:", facturation.getStatut());

        infoBox.getChildren().addAll(infoTitle, dateBox, methodeBox, statutBox);
        ...
        saveBtn.setStyle("-fx-background-color: #2ecc71; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 8px 16px;");
        saveBtn.setGraphic(new Label("💾"));
        saveBtn.setOnAction(event -> {
            try {
                double nouveauMontant = Double.parseDouble(montantField.getText());
                if (nouveauMontant <= 0) {
                ...
                confirm.setContentText("Vous êtes sur le point de modifier le montant de cette facturation.");

                Optional<ButtonType> result = confirm.showAndWait();
                if (result.isPresent() && result.get() == ButtonType.OK) {
                    facturation.setMontant(nouveauMontant);
        ...
        cancelBtn.setStyle("-fx-background-color: #e74c3c; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 8px 16px;");
        cancelBtn.setGraphic(new Label("✕"));
        cancelBtn.setOnAction(e -> modalStage.close());

        buttonBox.getChildren().addAll(saveBtn, cancelBtn);

        // Assemblage du formulaire
        formCard.getChildren().addAll(montantBox, new Separator(), infoBox);
        mainContainer.getChildren().addAll(titleBox, formCard, buttonBox);
    ...
    // Méthode utilitaire pour créer une ligne d'information non modifiable
    private HBox createInfoRow(String label, String value) {
        HBox row = new HBox(10);
        row.setAlignment(Pos.CENTER_LEFT);

        Label labelLbl = new Label(label);
        labelLbl.setStyle("-fx-font-size: 13px; -fx-text-fill: #7f8c8d;");

        Label valueLbl = new Label(value);
        valueLbl.setStyle("-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: #2c3e50;");
