...
public class ArticledetailsControllerP {
...
// Gestion de la sélection
    ...
    private void retourPagePrecedente(ActionEvent event) {
        try {
            ...
            Stage stage = (Stage) btnRetour.getScene().getWindow(); // Assurez-vous que 'nom' est un contrôle valide
            stage.setScene(new Scene(root));
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
        // Ici, tu peux fermer la fenêtre actuelle

        // OU si tu veux charger une autre page, fais-le ici !
        // Exemple pour aller vers une autre scène
    /*
    try {
        Parent root = FXMLLoader.load(getClass().getResource("/path/to/taPagePrecedente.fxml"));
    ...
    */
    ...
    private VBox creerCommentaireAvecReponses(Commentaire parent, List<Commentaire> tous, int indentation) {
        ...
        // Créer les icônes
        ImageView likeIcon = new ImageView(new Image(getClass().getResource("/images/like.png").toExternalForm()));
        ImageView dislikeIcon = new ImageView(new Image(getClass().getResource("/images/dislike.png").toExternalForm()));

// Ajuster la taille des icônes
        likeIcon.setFitHeight(18);
        likeIcon.setFitWidth(18);
        dislikeIcon.setFitHeight(18);
        dislikeIcon.setFitWidth(18);

// Créer les labels à côté des icônes
        Label likeCount = new Label();
        Label dislikeCount = new Label();

// Charger les valeurs
        try {
            likeCount.setText(String.valueOf(new ServiceCommentaire().countLikes(parent.getIdCommentaire())));
            dislikeCount.setText(String.valueOf(new ServiceCommentaire().countDislikes(parent.getIdCommentaire())));
        } catch (SQLException e) {
            e.printStackTrace();
        }

// Créer les conteneurs interactifs
        HBox likeBox = new HBox(5, likeIcon, likeCount);
        HBox dislikeBox = new HBox(5, dislikeIcon, dislikeCount);
        likeBox.setAlignment(Pos.CENTER_LEFT);
        dislikeBox.setAlignment(Pos.CENTER_LEFT);
        ...
        HBox votesBox = new HBox(10, likeBox, dislikeBox);
