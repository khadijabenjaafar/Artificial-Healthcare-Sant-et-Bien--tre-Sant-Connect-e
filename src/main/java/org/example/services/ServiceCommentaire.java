...
public class ServiceCommentaire implements IService<Commentaire> {
    ...
    public List<Commentaire> afficher() throws SQLException {
        ...
        String sql = "SELECT * FROM commentaire ORDER BY date_commentaire DESC"; // PAS id_article ici
        ...
        while (rs.next()) {
            ...
            if (parentIdObj != null) {
                c.setParentId(((Number) parentIdObj).intValue());
            } else {
                c.setParentId(0);
            }
    ...
    public int countByMonth(int month) throws SQLException {
        String sql = "SELECT COUNT(*) FROM commentaire WHERE MONTH(date_commentaire) = ?";
        PreparedStatement ps = connection.prepareStatement(sql);
        ps.setInt(1, month);
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
